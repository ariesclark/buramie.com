...
const ImageComponent: FC<ThumbnailImageProps> = (props) => {
  return <Image className='bg-black' {...props.imageProps} title="" height={props.item.height} width={props.item.width} alt="Picture of Buramie"/>
}
...
export default function Home() {
  ...
  return (
    <div className='relative w-screen h-screen flex items-center justify-center'>
      <Gallery thumbnailImageComponent={ImageComponent} margin={0} enableImageSelection={false} onClick={() => setPlaying((playing) => !playing)} images={images} />
      <video  className="hidden" src={"/rindo.mp4"} ref={reference}/>
      <div className='px-4 py-3 fixed justify-between flex flex-col sm:flex-row items-center bottom-0 backdrop-blur-sm backdrop-brightness-75 w-full'>
        <span>
          Made for Buramie with <span className='text-red-500'>❤</span>.
        </span>
        <div className='flex flex-col text-xs brightness-75 text-center sm:text-right'>
          <span>Images are property of their respective owners.</span>
          <span>&copy; {new Date().getFullYear()} <Link className='hover:border-current border-b-2 border-transparent' target="_blank" href="https://ariesclark.com">ariesclark.com</Link>, All rights reserved.</span>
        </div>
      </div>
    </div>
  )
